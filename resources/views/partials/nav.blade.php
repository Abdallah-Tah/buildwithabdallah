...
<header x-data="{ open: false }" @keydown.escape.window="open = false" class="sticky top-0 z-40 border-b border-line/70 bg-bg/95 backdrop-blur">
    <div class="mx-auto max-w-[1280px] px-6 lg:px-10 h-20 flex items-center justify-between">
        ...
        <div class="hidden lg:flex items-center gap-2">
            ...
            <script>
                (function(){
                    var t=document.documentElement.dataset.resolvedTheme||'dark';
                    var btn=document.querySelector('.theme-toggle');
                    if(!btn)return;
                    var sun=btn.querySelector('.dark-icon');
                    var moon=btn.querySelector('.light-icon');
                    function update(){
                        var r=document.documentElement.dataset.resolvedTheme||'dark';
                        if(r==='dark'){sun.classList.remove('hidden');moon.classList.add('hidden');}
                        else{sun.classList.add('hidden');moon.classList.remove('hidden');}
                    }
                    update();
                    new MutationObserver(update).observe(document.documentElement,{attributes:true,attributeFilter:['data-resolved-theme']});
                })();
            </script>
